...
**`routing`** is the article's entry in the stack's routing map (`index.md`), which is how `/stacks:ask` recognizes the right article without reading every body. Write ONE line, no line breaks, in the terms an asker would actually use — what the article covers and the questions it answers — not a restatement of the title. Lead with the concrete subject, then the questions. Plain text only (no `[[wikilinks]]`, no markdown, no leading `-`). Example for `vav-box-minimum-airflow`: `Minimum airflow/damper settings for VAV boxes — how low can the minimum go, what sets the floor, why low-load ventilation matters`.
