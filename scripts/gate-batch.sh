...
# Gate a batch of expected output paths: run assert-written then (optionally)
# assert-structure for each path; aggregate failures; exit 1 on any failure.
#
# Usage:
...
#                    structure check and run assert-written only.
...
FAILED=()
for path in "$@"; do
  if ! "$SCRIPT_DIR/assert-written.sh" "$path" "$DISPATCH_EPOCH" "$AGENT_LABEL" 2>/dev/null; then
    FAILED+=("$path")
