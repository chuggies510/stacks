...
| `scripts/pipeline/audit.sh {prep\|gate\|finish} {stack}` | Audit pipeline orchestration (0.47.0, #87 T7). `prep` = article enum + CAP=3 shard → `dev/audit/{run.env,dispatch.tsv}`; `gate` = gate-batch (`audit-findings`) + per-row RUN_ID check + `check-coverage.sh --verdict VALIDATED`; `finish` = report.md + soft-spots.tsv + cleanup. Per-article `VALIDATED` receipt replaced the `last_verified==today` date-gate (#71). `--self-check` (10 cases). |
