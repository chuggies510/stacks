...
| `scripts/check-coverage.sh [--field N] {dispatch.tsv} {output-file}...` | Per-item coverage gate (0.45.0, hardened 0.45.1). Reconciles the dispatch manifest (`batch_tag<TAB>item_id[<TAB>metadata]`) against receipt rows (item_id in col N, default 2); fails by name on omission, duplicate, unknown id, or missing/empty output file. Exit 0 only on exact set equality. Inline `--self-check` (10 red-when-broken cases). Wired into the enrich gate; audit/catalog pending (#71). Global-not-per-batch reconciliation gap: #92. |
| `scripts/pipeline/enrich.sh {prep\|gate\|finish} {stack} [--auto] [--query <text>]` | Enrich pipeline orchestration (0.46.0). `prep` = args + gap assembly + stale-check + CAP=5 shard → `dev/enrich/{run.env,dispatch.tsv}`; `gate` = gate-batch + check-coverage over gap_ids; `finish` = URL-dedup + cleanup. State via files, never shell env (#72). `--self-check` (6 cases). |
| `scripts/assert-structure.sh {path} {kind} {label}` | Content-shape gate. Kinds: `concept-batch`/`dedup-md` (`## Concept:` header), `dedup-meta` (`ALL_SLUGS=`), `article-md` (`title:` + `last_verified:`), `article-validated` (populated `last_verified:` date — read-and-fix validator sets no inline marks, #57). Covered by `tests/assert-structure.bats`. |
