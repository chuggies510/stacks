...
| `scripts/pipeline/enrich.sh {prep\|gate\|finish} {stack} [--auto] [--query <text>]` | Enrich pipeline orchestration (0.46.0). `prep` assembles gaps, shards at CAP=5, preserves tracked findings, and refuses a tracked active output collision; `gate` reconciles gap receipts; `finish` requires the manifest, aggregates only its batch files, and cleans untracked run state. State crosses phases through `dev/enrich/{run.env,dispatch.tsv}` (#72). `--self-check` (17 cases). |
| `scripts/pipeline/audit.sh {prep\|gate\|finish} {stack} [--full]` | Audit pipeline orchestration (0.47.0, #87 T7; incremental 0.52.0). `prep` applies hash-based incremental skip and CAP=5 sharding. `gate` checks batch freshness, RUN_ID, and exact `VALIDATED` coverage, then preflights every article and deterministically writes the quoted `last_verified` date through `article-field.sh`. `finish` rebuilds the report, carries soft spots, refreshes `verified.tsv`, and cleans run state. A truncated dispatch never stamps unaudited articles. `--self-check` (26 cases). |
| `scripts/pipeline/catalog.sh {queue\|prep\|gate-w1\|dedup\|gate-w2\|finish} {stack} [--from P]` | Catalog pipeline orchestration (0.48.0, #87 T8). `queue` = stacks with incoming, largest first; `prep` = `--from` stage + convert + W0 enum/paren-gate + W1 manifest; `gate-w1`/`gate-w2` = gate-batch over the 1:1 per-source/per-slug output files (path presence = coverage, so no check-coverage); `dedup` = W1b merge + W2 manifest + near-dup report; `finish` = tag-drift + W3 filing + W4 MoC + cleanup. RUN_ID_W2 captured at `dedup` is the single freshness floor for all W2 waves. State via files under `dev/extractions/`, never shell env (#72). Phases are manifest-authoritative (finish files/gates the dispatched set, not a live `find`; dedup rejects off-manifest batch files) — codex-hardened. `--self-check` (23 cases; 3 were false-failing on a `\t`-in-single-quotes grep pattern until 0.60.0 — see CLAUDE.md gotcha). |
...
| `scripts/dedup-extractions.py {extr_dir} {dedup_path}` \| `--self-check` | W1b dedup: merges concept blocks across `batch-*-concepts.md` by slug (union of `source_paths[]`, first-seen order), writes `_dedup.md`, one `_dedup-{slug}.md` per slug, and `_dedup-meta.txt` (`ALL_SLUGS`/`UPDATED_SLUGS`/`N_NEW`/`N_UPDATED`/`N_UNIQUE_CONCEPTS`/`INPUT_BLOCKS`/`NEAR_DUP_PAIRS` for the caller to source). The near-dup report (#106, 0.60.0) scores NEW-slug title pairs by **distinctive-token set cosine** — strips template tokens (in ≥3 titles) then compares the remainder, so it stops flagging shared title templates and stops missing real dups with differing titles; report-only, `--self-check` guards the metric. |
...
| `scripts/article-field.sh` \| `article_field {field} {article}` | **The** definition of reading and keyed replacement for a stacks article frontmatter field (0.78.0; writer added 0.78.5). The default reader returns one non-empty value. `--check-one` accepts an empty value but requires exactly one well-formed field in a closed frontmatter block; `--set` atomically replaces that field while preserving file mode. Article frontmatter is not YAML. Consumers: `regenerate-moc.sh` and the audit gate. |
| `scripts/regenerate-moc.sh {stack_root}` | Rebuilds `index.md` (Map of Contents) from article frontmatter **via `article-field.sh`** (0.78.0 — the inline awk reader is gone), grouping by `tags[0]`; appends each article's `routing:` line (`- [[slug\|title]] — {routing}`) to make it a recognition map (#59), bare link if absent; preserves the `## Reading Paths` section verbatim. W4 in catalog. |
