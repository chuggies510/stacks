...
| `scripts/article-field.sh` \| `article_field {field} {article}` | **The** definition of reading a stacks article's frontmatter field (0.78.0). Sourceable (`source article-field.sh`) or runnable directly. Prints nothing and returns non-zero unless the closing `---` was reached and the field is present, well-formed (`key: ` with the space) and non-empty; first occurrence wins; frontmatter must start at line 1. Article frontmatter is NOT YAML — do not reach for a YAML parser (27 of 1,174 corpus articles fail one). Sole consumer today: `regenerate-moc.sh`. |
