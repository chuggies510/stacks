...
- **Worker agents pin `model: sonnet` uniformly — now being measured, not yet re-tiered** (#95 / epic #109). Extraction has a gold-set benchmark (`dev/experiments/model-tier/extraction-benchmark.md`); the scope-map pattern (above) is what lets a cheaper tier clear it, but sonnet stays the reliable pick (haiku's cliff recall is variance-prone, 0.80↔1.0) and local models (qwen3-30b-a3b, gemma4-31b, a 122B-A10B straddle — scored by the peer session liminal, `results-liminal-S59.md`) clear the bar only behind a pal-chat harness (the Agent tool reaches only sonnet/haiku/opus/fable). All four stages now have an offline gold-set benchmark (S22 added synthesis, validation, enrichment alongside extraction, all in `dev/experiments/model-tier/`), each scored on its own discrimination axis: synthesis on over-claim (amplifying a thin claim), validation on poison-recall plus false-correction (symmetric — a miss ships poison, an over-trim corrupts truth), enrichment on false-CANDIDATE (a topical-but-non-grounding source accepted). Two codex adversarial passes (S22) hardened all four: the first found 10 defects in the three new benchmarks (score against the input block not the audited article; fix gameable metric denominators), the confirmation pass verified those fixes and audited extraction for the first time — its biggest catch was a reuse-precision floor that closes an emit-every-slug gaming hole (dumping all slugs scored perfect recall with no penalty). Every codex claim was verified against the corpus before applying. The recurring shape: every stage's cheap-tier failure is restraint under surface similarity, not transcription. The benchmark-design half of #109 is done; local scoring by liminal against the four benchmarks (and the live layers — enrichment search-recall, the validator shadow test vs sonnet) is the remaining work.
