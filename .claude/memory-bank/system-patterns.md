...
## Pipeline orchestration: per-pipeline prep|gate|finish scripts (epic #87, in progress)

The deterministic control flow of each fan-out pipeline is migrating out of SKILL.md Bash blocks into one checked-in script per pipeline (`scripts/pipeline/{catalog,audit,enrich}.sh`) with phase subcommands, because a bash script cannot spawn subagents so the flow is always `prep` → model dispatch (stays skill prose) → `gate` → `finish`. State crosses phases through files under `dev/<phase>/` (`run.env` KEY=VAL carrying `RUN_ID`; `dispatch.tsv` the coverage manifest `batch_tag<TAB>item_id[<TAB>metadata]`), never shell env. This is the structural fix for #72 and the home for the #71 coverage gate. Substrate decision (hybrid mirroring `ingest-book` Step 3B; Workflow fan-out deferred behind a measured prototype) is in `dev/specs/pipeline-orchestration-ssot.md` + `dev/plans/`. **Shipped so far: enrich (0.46.0).** Audit and catalog pending (T7/T8).

## Article contract SSOT (`references/article-contract.md`, 0.45.0)

One checked-in definition of the article-file frontmatter schema, bare source-ref form, per-source tier semantics, and the W1 concept-block shape, with `assert-structure.sh` named as enforcement — the same role `reference-tier.md` plays for the deep-reference tier. Five stages that used to restate (and drift on) the schema now point at it (STACK.md template, the four agent defs, catalog-sources, lookup). It records target-vs-current gaps honestly: `extraction_hash` is dead, `updated` has zero readers (#90), and tier-per-source_path is the target while `dedup-extractions.py` still collapses to one first-seen tier per slug (#89). Corpus migrations split from #77 into #88–#91.
...
- **Shell state does not persist between Bash blocks** (env vars/functions are lost; cwd persists). Structural fix (move orchestration into per-pipeline scripts) tracked in **#72 / epic #87**. **Partially migrated**: the enrich pipeline now runs through `scripts/pipeline/enrich.sh` (0.46.0, see Pipeline orchestration below) and carries no cross-block state; **audit and catalog are still hand-rolled** in their SKILL.md blocks (epic tasks T7/T8). See also the stacks CLAUDE.md gotcha.
- **Output gates prove a file was written, not that every dispatched item was processed** — the coverage mechanism now exists but is not yet applied everywhere. `scripts/check-coverage.sh` (0.45.0, hardened 0.45.1) reconciles a dispatch manifest against per-item receipt rows and fails by name on an omission, duplicate, unknown id, or missing file; it is wired into the **enrich** gate. **Audit still keys on `last_verified == today`** (a same-day rerun passes articles an aborted validator skipped) and **catalog W1/W2 still gate on files only** — both migrate under **#71 / epic #87 (T7/T8)**. Global-vs-per-batch reconciliation gap tracked in **#92**.
