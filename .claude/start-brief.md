...
Distilled 2026-06-12 from:
  tech-context.md @ 3e48117822a2186f0144881e64755ab3831ac124
  system-patterns.md @ c97ae2216c2e3e50975319435f9ce39ff632e053
...
- Skill frontmatter uses only `name` + `description`; description starts with "Use when...". No `version`/`allowed-tools`/`thinking`.
- Agent frontmatter: `tools` (comma-separated), `model`, `description`; include 3+ worked examples in the prompt.
...
### Core architecture
Three-layer plugin: (1) skills (user-facing SKILL.md walkthroughs: init-library, new-stack, catalog-sources, audit-stack, process-inbox, ask); (2) agents (5 workers: concept-identifier, article-synthesizer, validator, synthesizer, findings-analyst; plus orchestrators that shard worker dispatch via the Task tool); (3) templates (`templates/library/`, `templates/stack/` copied into user repos). The plugin holds no knowledge; it manipulates user-owned library repos.
...
- Articles are flat (no typed subdirs), 300-800 word soft cap, `[[wikilinks]]` via deterministic post-write bash pass (not agent output).
- Pipeline helpers: `assert-written.sh {path} {epoch} {label}` (write-or-fail), `wikilink-pass.sh {articles} {glossary}` (first-occurrence wrap), `compute-extraction-hash.sh` (sha256 of sorted source paths + slug; anchors the skip-list flywheel), `normalize-tags.sh {root}` (halts on out-of-vocab tags).

### Catalog pipeline (W0 to W4)
`/stacks:catalog-sources [stack]`: W0 enumerate `sources/incoming/` -> W0b skip list from `dev/audit/findings.md` -> W1 concept-identifier (parallel, `SOURCES_PER_AGENT=10`, slug immutability) -> W1b bash slug-collision dedup + `extraction_hash` via `scripts/compute-extraction-hash.sh` -> W2 article-synthesizer (parallel per concept, copies `extraction_hash` verbatim) -> W2b wikilink pass -> W2b-post tag drift check (`normalize-tags.sh` vs `STACK.md` `allowed_tags:`) -> W3 source filing -> W4 MoC regen preserving `## Reading Paths`.

### Audit pipeline (A1 to A5) with convergence loop
`/stacks:audit-stack {stack}`: A1 validator inline-marks articles `[VERIFIED]/[DRIFT]/[UNSOURCED]/[STALE]` -> A2 synthesizer writes `glossary.md`/`invariants.md`/`contradictions.md` -> A2b wikilink pass -> A3 findings-analyst writes `dev/audit/findings.md` (schema v2, id=SHA256, terminal `failed` status, carry-forward) -> A4 convergence check (2 empty passes or `MAX_AUDIT_PASSES` default 3) -> A5 on convergence `cp` (not `mv`) to `dev/audit/closed/{date}-findings.md` so active findings.md persists.
...
- Orchestrator wrapper: scale-sensitive dispatches run through an orchestrator that owns dispatch math, the per-output gate loop, and a summary JSON. Sub-agents hit "Prompt is too long" without sharding; main-session bash arrays do not persist across dispatch boundaries.
- Summary-JSON contract (0.13.0): orchestrators write `dev/{audit,extractions}/_{wave}-summary.json` with `{schema_version, wave, status, counts{}, epochs{}}`. Receipt `ORCHESTRATOR_OK: wave={id}` on stdout signals success. Gates grep the receipt then `jq -e` nested `.counts.FIELD` (never `jq -e '.a and .b'` since `0` is jq-falsy).
- Slug immutability: W1 matches concepts by claim overlap and reuses the existing slug; combined with W1b dedup, eliminates silent overwrite by parallel W2 writes.

### Known weak spots
gawk dependency (W1b dedup, W4 MoC; mawk fallback noted but unimplemented); audit-stack outer pass loop re-enters textually (not all models loop deterministically without operator re-invoke); cross-stack retrieval in `/stacks:ask` is a stub (one-stack scope only, blocks #5/#18).
