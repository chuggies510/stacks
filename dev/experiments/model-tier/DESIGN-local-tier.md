...
| **Validation** | judge ONE claim vs its cited source | per-claim isolation + **citation-presence gate** (`claim-citation-gate.sh`, STEP 1 as code — CITED/UNCITED, wired into `validator-shadow.sh` so an uncited claim can never be judged CLEAN) + cloud **`validation-verifier`** grades the local verdicts (poison recall / false-correction, derived from components) | per-claim recall 1.00 / false-corr 0.00 / byte-DET; item-6 add-citation miss **closed by the gate** (gated, qwen returns add-citation not CLEAN — S25) | gate wired + verifier/summary built (S25); real-model shadow pending |
| **Extraction** | describe a concept | deterministic **slug pre-match** (`slug-prematch.sh`: exact/normalized collision → REUSE, token-containment → NEAR) + cloud **`extraction-verifier`** grades every NEAR/NEW against `index.md`'s scope map for the **semantic** fragments the gate can't see (zero-shared-token over-mints, e.g. `agent-rl-fine-tuning` ⊂ `agent-harness-engineering`) | over-mint was the granularity meta-judgment; gate closes lexical fragments, verifier closes semantic ones | gate + verifier + summary built (S25); item-3 cliff shadow pending |
| **Enrichment** | ground ONE gap in a source (local loop: query→search→fetch→judge) | **URL dedup as set-membership** (`url-dedup-gate.sh`: normalized `candidate_url in filed_urls` → DUP in code, skip the call) + cloud **`enrichment-verifier`** re-fetches each CANDIDATE and grades `grounding_valid`/`tier_ok` | grounding 3/3, false-CANDIDATE 0/2, byte-DET; the ONLY miss was the dedup it was handed | gate + verifier + summary built (S25); full loop proven S59/S60; live-batch proof needs the local web-tool wiring |
