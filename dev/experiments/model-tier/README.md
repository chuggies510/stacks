...
| Validation | validator | `validation-benchmark.md` | **Retrieval build wired (S26, opt-in in audit Step 4.5).** `pair-claims.py` splits each article into claims and pulls each claim's OWN cited-source excerpt (token-overlap, top-K); the model judges one claim + one excerpt (the offline shape). **Gold-check clears both floors: poison recall 3/3, false-correction 0/4 on automated pairing** (`shadow-validate-run.sh --gold-check`). This retracts the earlier "false-corrects heavily" read — that was the harness confound (source starvation + model-picked excerpt reuse), not qwen. Uncited-CLEAN coercion now keys on pair-claims' ground-truth `cited` flag (also closes the echoed-claim gate limitation). Live full-`llm`-stack re-measure via the cloud verifier is the scale confirmation. |
