...
self_check() {
  ...
  # 3 graded drafts: one clean, one with an over-claim (floor breach), one with a
  # recall miss (floor breach) + citation fixes.
  ...
  local fail=0
  grep -q 'graded: 3' <<<"$out"            || { echo "FAIL: graded count"; fail=1; }
  grep -q 'clears floors: 1/3' <<<"$out"   || { echo "FAIL: clears rate"; fail=1; }
  grep -q 'over-claims (total): 2' <<<"$out" || { echo "FAIL: over-claim total"; fail=1; }
  grep -q 'recall misses: 1' <<<"$out"     || { echo "FAIL: recall misses"; fail=1; }
...
run() {
  ...
  jq -s '
    {
      graded:        length,
      clears:        (map(select(.clears_floors == true)) | length),
