...
Next per the benchmark's stage order: synthesis-first is the clean pilot (this result). Validator:
the S59-green was disputed (a peer re-score got ~0.58, a separate liminal agent got a deterministic
bare CLEAN), but the item-3/{2,3,5} prompt A/B (results-liminal-S61-validator-ab.md) then found
qwen3-30b-a3b at poison recall 1.00 on BOTH pre- and gate-first prompts, deterministic — so the
gate-first prompt did NOT regress this local model, and the reported miss is model- or context-
specific (likely the shipped sonnet agent, or full-article context vs the isolated benchmark item),
still open. Under the local-first/cloud-authoritative/log-the-diff pivot every stage is safe to
pilot regardless (sonnet ships), so validator can ride shadowed. Enrichment last. Caution worth
carrying: byte-DET is not a quality signal — a determinism check blesses a deterministic-wrong
verdict as "stable/passing" while it ships poison.
