...
Tell the user: "Stack '$STACK_NAME' scaffolded at $STACK_NAME/. Edit $STACK_NAME/STACK.md to define your scope, source hierarchy, topic template, and filing rules. This Hermes release cannot catalog sources; use a compatible Stacks harness when the stack is ready."
...
Report: "Stack '$STACK_NAME' created. Drop sources in $STACK_NAME/sources/incoming/ for a compatible Stacks harness to catalog into article-per-concept wiki entries. Note: sources/incoming/ is gitignored staging: those files are not tracked and do not sync across machines."
